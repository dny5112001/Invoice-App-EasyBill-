...
const EstimateEditingPage = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { estimateNumber } = route.params;
  ...
  const handleSave = async () => {
    ...
    const updateResult = await updateEstimate(estimateNumber, estimateData);
    if (updateResult) {
  ...
  }, [setBusinessName]); // Added setBusinessName to dependencies

  const fetchEstimateData = useCallback(async () => {
    try {
      const estimateData = await getIndividualEstimate(estimateNumber);
      const estimate = estimateData[0];
      if (estimate) {
        setEstimateNumber(estimate.estimateNumber);
        setCreationDate(
          estimate.creationDate ? new Date(estimate.creationDate) : new Date()
        );
        setDueDate(estimate.dueDate ? new Date(estimate.dueDate) : null);
        setClientEmail(estimate.clientEmail);
        setDiscountType(estimate.discountType);
        setDiscount(estimate.discount);
        setTaxName(estimate.taxName);
        setTaxRate(estimate.taxRate);
        setShippingAmount(estimate.shippingAmount);
        setItems(JSON.parse(estimate.items));
        setTerms(estimate.terms);
        setSignatureName(estimate.signatureName);
        setSignatureImage(estimate.signatureImage);
        setStatus(estimate.status);
        setPaymentMethod(estimate.paymentMethod);
        setBusinessName(estimate.businessName);
      }
    } catch (error) {
      ...
      >
        <View style={{ gap: 10, marginVertical: 20 }}>
          ...
          >
            <Text style={{ color: creationDate ? "#000" : "#aaa" }}>
              Creation Date : {formatDate(parseDate(creationDate))}
            </Text>
          </TouchableOpacity>
          {showCreationPicker && (
            <DateTimePicker
              value={parseDate(creationDate) || new Date()}
              mode="date"
              display="default"
              onChange={(event, selectedDate) => {
                setShowCreationPicker(false);
                if (selectedDate) setCreationDate(selectedDate.toISOString());
              }}
            />
          )}
          ...
          >
            <Text style={{ color: dueDate ? "#000" : "#aaa" }}>
              Due Date : {formatDate(parseDate(dueDate))}
            </Text>
          </TouchableOpacity>
          {showDuePicker && (
            <DateTimePicker
              value={parseDate(dueDate) || new Date()}
              mode="date"
              display="default"
              onChange={(event, selectedDate) => {
        ...
        >
          ...
          <View style={{ marginTop: 20, gap: 10 }}>
            ...
            >
              <Text>Subtotal</Text>
              <Text>₹{(Number.parseFloat(subTotal) || 0).toFixed(2)}</Text>
            </View>
            <Pressable
              style={{ flexDirection: "row", justifyContent: "space-between" }}
          ...
          >
            <Text>Total</Text>
            <Text>₹{(Number.parseFloat(totalAmount) || 0).toFixed(2)}</Text>
          </View>
        </View>
        {/* Signature */}
        <View style={{ marginTop: 10 }}>
        ...
        <View style={{ marginTop: 10 }}>
          ...
          >
            <View>
              ...
              <Text style={{ color: "#000" }}>
                {terms ? "Edit Terms & Conditions" : "Add Terms & Conditions"}
              </Text>
            </View>
            <Icon name="chevron-right" size={24} color="#000" />
          </TouchableOpacity>
              ...
              <SafeAreaView style={styles.safeArea}>
                <View style={styles.modalContent}>
                  ...
                  <View style={styles.inputContainer}>
                    <TextInput
                      style={styles.input}
                      placeholder={`Enter ${discountType} discount`}
        ...
        >
          <TouchableOpacity
            ...
            onPress={() => {
              navigation.navigate("Estimate Preview", { estimateNumber });
            }}
          >
          ...
          >
            <Text style={{ color: "#10b981", fontSize: 18 }}>Update</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
...
export default EstimateEditingPage;
...
const styles = StyleSheet.create({
  ...
  // ... (rest of the styles remain the same as in EstimateCreationPage)
  modalContent1: {
